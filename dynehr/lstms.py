...
class LSTM(nn.Module):
	...
	def forward(self, x):
		h = torch.zeros(x.size(0), self.hidden_dim, dtype=torch.float32).to(self.device)
		c = torch.zeros(x.size(0), self.hidden_dim, dtype=torch.float32).to(self.device)
